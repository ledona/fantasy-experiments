...
```
5. Selenium navigation (i.e. the python script) may fail regularly. There are 2 things to try for getting more reliablility and having the script run longer before stopping. a) Resize/Zoom the browser so that the entire navigational UI is visible. b) Run the script in a loop so that when it fails it will immediately restart and pick up were it left off. e.g. to run the command 5 times on windows in powershell execute:
```
...
```
6. (optional) If debugging/running under VSCODE
