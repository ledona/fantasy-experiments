...
# no more than 3 processes for 12GB RAM
dumpdata.sc --seasons $SEASONS --no_team --only_starters \
   ...
   --calc_stats "*_score" \
   --current_extra "hitter_*" indoors is_home elo_win_prob \
   ...
   --dask_mode processes --dask_tasks 3 \
...
dumpdata.sc --seasons $SEASONS --no_team \
   ...
   --calc_stats "*_score" \
   --current_extra sp_hand elo_win_prob indoors is_home "odds_*" venue "weather_*" \
...
dumpdata.sc --seasons $SEASONS --no_player \
   ...
   --current_extra $TEAM_CURRENT_X elo_win_prob indoors is_home \
